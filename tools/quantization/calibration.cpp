...
void Calibration::_fake_invert_quant_weights() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        uint32_t bufferSzie = 0;
        {
            flatbuffers::FlatBufferBuilder builder;
            auto offset = MNN::IDSTQuan::Pack(builder, param->quanParameter.get());
            builder.Finish(offset);
            auto quanParaBuffer = builder.GetBufferPointer();
            auto size = builder.GetSize();
            auto newParam = flatbuffers::GetRoot<MNN::IDSTQuan>(quanParaBuffer);

            common = ConvolutionCommon::load(newParam, false, true);
            buffer = common->weight.get();
            bufferSzie = common->weight.size();
        }
