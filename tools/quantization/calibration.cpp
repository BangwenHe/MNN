...
void Calibration::_fake_invert_quant_weights() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        size_t bufferSzie = 0;
        {
            ...
            auto offset = MNN::Convolution2D::Pack(builder, param);
            builder.Finish(offset);
            auto quanData = builder.GetBufferPointer();
            auto quanDataSize = builder.GetSize();
            auto conv = flatbuffers::GetRoot<MNN::Convolution2D>(quanData);

            common = ConvolutionCommon::load(conv, nullptr, false, true);
